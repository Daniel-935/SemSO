...
procesosNuevos = generaProcesos(numProcesos)[:]
...
while len(procesosListos) > 0 or len(procesosBloqueados) > 0 or len(procesosNuevos) > 0:
  ...
  #!Ejecuta el proceso
  while procesoEjecutar["tiempoRestante"] > 0:
    ...
    #*Muestra la tabla
    linea = getTablaEjecucion(procesosListos, procesosBloqueados, procesosNuevos, procesosTerminados, procesoEjecutar, contadorGlobal)
    sys.stdout.write('\033[H')
